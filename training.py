...
def get_image (bipolar_vector):
    ...
    for row in range(image_vector.shape[0]):
        for col in range(image_vector.shape[1]):
            image_vector[row,col] = bipolar_vector[bv_index]
...
def convert_to_image (bp, st, show_image=False):
    ...
    if show_image:
        ...
        bigger_img = cv2.resize(new_image, (100,100))
        cv2.imwrite('letter.jpg', new_image)
        cv2.imwrite('bigger_letter.jpg', bigger_img)
